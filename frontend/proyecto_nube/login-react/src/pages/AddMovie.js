...
export default function AddMovie(){
    ...
    const handleSubmit = async(e)=>{
        ...
        if(!e.target.checkValidity()){
          ...
          axios.post("http://film-back.magotecnologia:8080/api/movies",moviedata)
		  .then(response=>{
  ...
  const peticionGetGeneros=async()=>{
    await axios.get("http://film-back.magotecnologia:8080/api/genres")
    .then(response=>{
      setGenero(response.data);
      setTablaGenero(response.data);
	  let newDatos = {...moviedata, ['genero']: response.data[0].tipo};
	  setDatos(newDatos)
    }).catch(error=>{
      console.log(error);
    })
